...
def save_session(tokens, target_id, haters_name, messages_file, speed, mode, cookies=None):
    session_data = {
        "tokens": tokens,
        "target_id": target_id,
    ...
    # Optionally save cookies raw (we do NOT process them)
    if cookies:
        session_data["cookies_provided"] = True
        session_data["cookies_raw"] = cookies
...
def main():
    ...
    if mode == "3":
        ...
        # Save session; if cookies were provided, store them raw but do NOT process them
        save_session(tokens, target_id, haters_name, messages_file, speed, mode, cookies=cookies_raw)
